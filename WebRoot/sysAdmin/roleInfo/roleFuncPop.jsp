...
function save(){
	var url = "doRoleInfo.do?method=insertRoleFuncInfo&roleCode=<%=request.getAttribute("roleCode")%>&factoryCode=<%=request.getAttribute("factoryCode")%>&funcs="+ckBoxsStr()+"&"+Math.random();
...
<body>
<form name="frmGo" method="post" action="doRoleInfo.do?method=queryRoleInfo&pop=true&atTag=<%=request.getParameter("atTag")%>">
