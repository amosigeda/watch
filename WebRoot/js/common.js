...
function pmwin(action, param,clayer) {
	...
	if(action == 'open') {
		...
		if(!$_(layer)) {
			...
			$_(layer).innerHTML = '<div class="showWin" style="width: '+winwidth+'px;position: relative;top:'+topheight+'px;">' +
				'<a href="javascript:void(0)" onclick="pmwin(\'close\',\'\',\''+layer+'\');"title="Close" style="position: absolute; right: 12px; top: 12px"><img src="/skin/m/img/c.gif"></a>' +
		...
		//alert($_(layer).innerHTML);
		$_(layer).style.display = '';
		$_(layer).style.top = ((clientHeight - pmheight) / 2 + scrollTop) + 'px';
		if(!param) {
			$_(winmask).innerHTML = 'Error!';
		} else {
...
function setAjaxShow(xmlHttp, id, loading) {
	if (xmlHttp.readyState < 4) {
		if (!loading) {
			$_(id).innerHTML='<img src="/skin/m/img/loading.gif" align="absmiddle"> 请稍后...';
		}
	}
	if (xmlHttp.readyState == 4) {
...
function selectPowerAll(pid) {
	var adminpower = document.getElementsByName('adminPower['+pid+'][]');
	var ischecked = 0;
	if($_(pid).checked == 1) ischecked = 1;
	for(var i = 0;i < adminpower.length; i ++) {
		adminpower[i].checked = ischecked;
		selectPowerAll(adminpower[i].id);
	}
}	

function selectPowerIsOn(pid) {
	var adminpower = document.getElementsByName('adminPower['+pid+'][]');
	var ison = 0;
	for(var i = 0;i < adminpower.length; i ++) {
		if (adminpower[i].checked == 1) ison = 1;
	}
	return 	ison;
}


function selectPower(pid,pidn,pidnArr,type) {
	selectPowerAll(pid);
	if (type >= 4 && $_(pid).checked == 0) return;//局部权限若取消选择不影响上级
	var pidnArrlength = pidnArr.split(",");
	pidnArrlength.reverse();
	for(var i = 0;i < pidnArrlength.length; i ++) {
		$_('p'+pidnArrlength[i]).checked = selectPowerIsOn('p'+pidnArrlength[i]);
	}	
