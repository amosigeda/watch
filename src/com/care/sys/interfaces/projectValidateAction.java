...
public class projectValidateAction extends BaseAction{
	
	Log logger = LogFactory.getLog(projectValidateAction.class);
	...
	throws Exception {
		...
		try{
			...
			/*	vo.setProjectName(projectName);
				vo.setStatus(status);*/
				vo.setCondition("project_name='"+projectName+"'and status='"+1+"'");
				
				List<DataMap> listPi=pfa.getProjectInfo(vo);
				if(listPi.size()>0){
