...
public class disturbInfoAction extends BaseAction {
	Log logger = LogFactory.getLog(disturbInfoAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (user_id.equals("-1")) {
				...
				deviceActiveInfo.setCondition("serieNo ='" + serial_number
						+ "' and belong_project='" + belongProject + "'"); // 鍒ゅ畾鏉′欢
				List<DataMap> deviceList = ServiceBean.getInstance()
						.getDeviceActiveInfoFacade()
						.getDisturbInfo(deviceActiveInfo);
				int size = deviceList.size();
			for(int i=0;i<size;i++) {
				String repeat =(String) deviceList.get(0).getAt("repeat");
				String time = (String) deviceList.get(0).getAt("time");
				String status = (String) deviceList.get(0).getAt("status");	
				if (!"".equals(repeat) && !"".equals(time) && !"".equals(status)) {
