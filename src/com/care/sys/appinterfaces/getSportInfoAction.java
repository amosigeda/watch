...
public class getSportInfoAction extends BaseAction{
	Log logger = LogFactory.getLog(getSportInfoAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try{			
			...
			if(serieNo != null && !serieNo.equals("") && type != null && !type.equals("")
					&& bg != null && !bg.equals("") ){
				DeviceActiveInfo vo = new DeviceActiveInfo();
				vo.setCondition("imei = '"+serieNo+"' and b_g='"+bg+"'and sport_type='"+type+"'");
				vo.setOrderBy("upload_time");
				List<DataMap> list = facade.getSportTypeInfo(vo);
				if(list.size() <= 0){   //说明后台没有记录
					json.put("resultCode", 0);  //记录-1
				}else{
					json.put("energy", list.get(0).getAt("energy")+"");
					json.put("step",list.get(0).getAt("distance")+"");
